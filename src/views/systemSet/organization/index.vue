...
import { getFwOrgList, getFwOrgById, deleteFwOrgById, updateFwOrg, addFwOrg, getFwOrgListById } from '@/api/systemSet/organization'
...
export default {
  ...
  methods: {
    ...
    editMenu: function(row) {
      getFwOrgById({ menuId: row.MENU_ID }).then(res => {
    ...
    deletMenu(row) {
      deleteFwOrgById({ menuId: row.MENU_ID }).then(response => {
        if (response.data) {
          this.$message({
    ...
    menuFormAction() {
      this.$refs['menuForm'].validate((valid) => {
        if (valid) {
          if (this.formType === 1) {
            addFwOrg(this.menuForm).then(res => {
              if (res.data) {
                this.$message({
            ...
            updateFwOrg(this.menuForm).then(res => {
              if (res.data) {
                this.$message({
